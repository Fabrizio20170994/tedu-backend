...
export class CommentFileService {
    constructor (
        @InjectRepository(CommentFileEntity) private commentFileRepository: Repository<CommentFileEntity>,
    ...
    async commentFile(userId: number, data: Partial<FileDTO>, commentId: number) {
        ...
        if (comment.user.id != userId) {
            throw new UnauthorizedException("Este comentario no pertenece al usuario actualmente logeado")
        }
