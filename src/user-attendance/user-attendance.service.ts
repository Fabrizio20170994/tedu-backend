...
export class UserAttendanceService {
  ...
  async registerAttendance(
    ...
    //actualizar
    const asistenciaLista = await this.attendanceRepository.findOneOrFail(
      attendance_id,
    );
    if (asistenciaLista.registered) {
      throw new BadRequestException(
        {
          registered: asistenciaLista.registered,
          attendance_date: asistenciaLista.attendance_date,
        },
        'Esta asistencia ya está marcada como "registrada"',
      );
    }
    var created = 0;
    for (const userAElement of data) {
      const user = await this.userRepository.findOneOrFail(
        userAElement.student_id,
      );
  ...
  async editAttendance(
    ...
    if (asistenciaLista.registered == false) {
      throw new BadRequestException(
        ...
        'No se puede editar una asistencia que no ha sido "registrada"',
      );
    }
    var updatedRows: number = 0;
    for (const element of data) {
      const editUserAttendance: editUserAttendanceDTO = {
        attended: element.attended,
      };
      const user = await this.userRepository.findOneOrFail(element.student_id);
      const userAttendance =
        this.userAttendanceRepository.create(editUserAttendance);
      userAttendance.user = user;
      userAttendance.attendance = asistenciaLista;
      const newUserAttendance = await this.userAttendanceRepository.save(
        userAttendance,
      );
      if (newUserAttendance) {
        updatedRows += 1;
      }
    }
    if (updatedRows > 0) {
